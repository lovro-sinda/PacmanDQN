...
class DQN:
    def __init__(self, params):
        ...
        self.c1 = tf.nn.conv2d(input=self.x, filters=self.w1, strides=[1, stride, stride, 1], padding='SAME',name=self.network_name + '_'+layer_name+'_convs')
        ...
        self.c2 = tf.nn.conv2d(input=self.o1, filters=self.w2, strides=[1, stride, stride, 1], padding='SAME',name=self.network_name + '_'+layer_name+'_convs')
        ...
        self.Q_pred = tf.reduce_sum(input_tensor=tf.multiply(self.y,self.actions), axis=1)
        self.cost = tf.reduce_sum(input_tensor=tf.pow(tf.subtract(self.yj, self.Q_pred), 2))
